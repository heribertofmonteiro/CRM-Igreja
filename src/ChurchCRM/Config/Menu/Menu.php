...
class Menu
{
    ...
    private static function buildMenuItems(): array
    {
        return [
            ...
            'Financeiro'   => self::getFinanceiroMenu(),
            'Ministerio'    => self::getMinisterioMenu(),
    ...
    private static function getFinanceiroMenu(): MenuItem
    {
        ...
        $financeiroMenu = new MenuItem(gettext('Financeiro'), 'v2/financeiro', ($hasFinancePermission || $hasFundraiserPermission), 'fa-money-bill-wave');
        
        // Submenu de Depósitos
        if ($hasFinancePermission) {
            $depositsSubMenu = new MenuItem(gettext('Deposits'), '', true, 'fa-cash-register');
            $depositsSubMenu->addSubMenu(new MenuItem(gettext('View All Deposits'), 'FindDepositSlip.php'));
            $depositsSubMenu->addSubMenu(new MenuItem(gettext('Deposit Reports'), 'FinancialReports.php'));
            if (isset($_SESSION['iCurrentDeposit']) && $_SESSION['iCurrentDeposit'] > 0) {
                $depositsSubMenu->addSubMenu(new MenuItem(gettext('Edit Deposit Slip'), 'DepositSlipEditor.php?DepositSlipID=' . $_SESSION['iCurrentDeposit']));
            }
            $depositsSubMenu->addCounter(new MenuCounter('iCurrentDeposit', 'bg-green', $_SESSION['iCurrentDeposit'] ?? 0));
            $financeiroMenu->addSubMenu($depositsSubMenu);
        }
        
        // Submenu de Fundraisers
        if ($hasFundraiserPermission) {
            $fundraiserSubMenu = new MenuItem(gettext('Fundraiser'), '', true, 'fa-money-bill-alt');
            $fundraiserSubMenu->addSubMenu(new MenuItem(gettext('Create New Fundraiser'), 'FundRaiserEditor.php?FundRaiserID=-1'));
            $fundraiserSubMenu->addSubMenu(new MenuItem(gettext('View All Fundraisers'), 'FindFundRaiser.php'));
            if (isset($_SESSION['iCurrentFundraiser']) && $_SESSION['iCurrentFundraiser'] > 0) {
                $fundraiserSubMenu->addSubMenu(new MenuItem(gettext('Edit Fundraiser'), 'FundRaiserEditor.php?FundRaiserID=' . $_SESSION['iCurrentFundraiser']));
            }
            $fundraiserSubMenu->addSubMenu(new MenuItem(gettext('Add Donors to Buyer List'), 'AddDonors.php'));
            $fundraiserSubMenu->addSubMenu(new MenuItem(gettext('View Buyers'), 'PaddleNumList.php'));
            $iCurrentFundraiser = $_SESSION['iCurrentFundraiser'] ?? 0;
            $fundraiserSubMenu->addCounter(new MenuCounter('iCurrentFundraiser', 'bg-blue', $iCurrentFundraiser));
            $financeiroMenu->addSubMenu($fundraiserSubMenu);
        }
        
        // Submenu de Relatórios
        if ($hasFinancePermission) {
            $reportsSubMenu = new MenuItem(gettext('Reports'), '', true, 'fa-chart-bar');
            $reportsSubMenu->addSubMenu(new MenuItem(gettext('Financial Reports'), 'FinancialReports.php'));
            $reportsSubMenu->addSubMenu(new MenuItem(gettext('Tax Report'), 'TaxReport.php'));
            $financeiroMenu->addSubMenu($reportsSubMenu);
        }
    ...
    private static function getMinisterioMenu(): MenuItem
    {
        $hasPermission = AuthenticationManager::getCurrentUser()->isAdmin() || 
                         AuthenticationManager::getCurrentUser()->isEditRecordsEnabled();
        $ministerioMenu = new MenuItem(gettext('Ministério'), 'v2/ministerio', $hasPermission, 'fa-hands-helping');
        
        $ministerioMenu->addSubMenu(new MenuItem(gettext('Ministérios'), 'v2/ministerio', $hasPermission));
        $ministerioMenu->addSubMenu(new MenuItem(gettext('Reuniões'), 'v2/ministerio/reunioes', $hasPermission));
        $ministerioMenu->addSubMenu(new MenuItem(gettext('Mensagens'), 'v2/ministerio/mensagens', $hasPermission));
        
        return $ministerioMenu;
    }
